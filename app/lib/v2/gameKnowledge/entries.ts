...
export const GAME_KNOWLEDGE_ENTRIES: readonly KnowledgeEntry[] = [
  ...
  },
  {
    ...
    summary:
      "ゲームルール上、Playerは任意のTurnに任意額を配当できる。Standard AIだけが「年度末Q4のみ・その年度の年間純利益の {{dividendBasePayoutPercent}}%」という保守的な自社ポリシーで動く。",
    explanation: [
      ...
      "【Standard AIのポリシー（年間純利益ベース）】Standard AIは年度末（Q4）にだけ配当を検討し、次の条件をすべて満たす場合のみ配当する:",
      "  1. 年度末（Q4）である  2. 財務健全性が healthy  3. Crisis State が NORMAL",
      "  4. 当期の新規CAPEX提案が0件  5. 分配可能利益 > 0  6. 配当可能上限 > 0",
      "配当額はQ4の決算が確定した**後**に精算する。年間配当目標 = max(0, その年度Q1〜Q4の当期純利益の合計) × {{dividendBasePayoutRatio}}（経営性格により ±5% の範囲で前後する）。",
      "そこから同年度に既に支払った配当（Playerの金額指定を含む実支払額）を差し引いた額を、上限 min(現金, 分配可能利益) の範囲で支払う。",
      "年度の途中の四半期（Q1〜Q3）に赤字があっても、年間合計が黒字なら配当対象になりうる。逆に年間合計が0以下なら無配（既に払った配当を返還することはない）。",
      "配当の算定基準は年間の当期純利益という**フロー**であり、累計の分配可能利益（ストック）ではない。ストックは上限としてのみ働く。",
      "Q4で条件を満たさなければその年度は無配であり、翌Q1へ繰り越して配当することはない。未達額を翌年度へ繰り越すこともしない。",
    ].join("\n"),
    ruleType: "CONSTRAINT",
    sourceType: "CODE",
    sourceReference:
      "app/lib/v2/finance/dividend.ts (computeMaxDividendUsd / resolveDividendDecision), app/lib/v2/finance/annualDividend.ts (年間精算), app/lib/v2/companyLab/standardAi/decision/dividend.ts, docs/v2/standard_ai_dividend_policy_div3_div4.md",
    appliesToRoles: ["CFO", "CEO"],
    keywords: ["配当", "dividend", "株主還元", "payout", "分配可能利益", "年間純利益", "年度末精算"],
    ...
    semanticWarnings: [
      ...
      "配当の算定基準を「直近四半期（Q3）の純利益」と説明しない。基準は年度Q1〜Q4の合計である。",
    ],
    version: V,
  },
