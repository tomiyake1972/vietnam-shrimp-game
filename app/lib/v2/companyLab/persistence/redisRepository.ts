// ShrimpX V2 — 会社ラボ専用永続化 Redis Repository（Phase 8C-1）
//
...
import { assertAllowedCompanyLabKeys } from "../../redis/companyLabRedisKeyGuard";
...
import { CompanyLabStateRepository, CompanyLabQuarterCommitInput, CreateCompanyLabInput, assertCommitInputInvariants } from "./repository";
...
export function createCompanyLabStateRepository(deps: CompanyLabStateRepositoryDependencies): CompanyLabStateRepository {
  ...
  async function createLab(input: CreateCompanyLabInput): Promise<CompanyLabPersistedStateV1> {
    ...
    const key = keysFor(input.labId).current;
    assertAllowedCompanyLabKeys([key], appEnv, input.labId);
