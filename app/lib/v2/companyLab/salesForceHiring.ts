...
//   一方、営業人員の採用は「今期採用した人数は今期は使えず、教育・引継ぎ期間を
//   経て次の四半期から営業活動に参加する」という1四半期分の反映遅延を持つ。
//   このため意思決定側は「今期の新規採用人数（0以上の増分）」という差分のみを
//   持ち、当期の配置可能人数（バジェット検証・実績人件費のいずれも）には一切
//   加算しない。四半期が確定した時点で初めて、次期の状態へ加算する
