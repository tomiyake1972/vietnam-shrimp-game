...
export interface BriefingBuildInput {
  ...
  /**
   * 【Dynamic Scenario 1由来】そのターン時点で人間プレイヤーが読めるシナリオNews
   * （scenarioNews.tsが射影済み）。News は特定役員の専門領域ではなく4人全員が同じ記事を
   * 読んでいる前提の共有情報のため common へ入れる。省略時は空配列（News を持たない
   * シナリオ・既存の呼び出し側との互換のため）。
   */
...
export function buildExecutiveBriefingPacket(input: BriefingBuildInput): ExecutiveBriefingPacket {
  const { context, previousQuarter, playerDraft, contracts, receivables, payables, loans, capexProjects, borrowingHeadroom, crisis, financialHistory, operationalHistory, scenarioNews } = input;
