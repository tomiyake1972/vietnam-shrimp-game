...
test("AMM-8: briefingは既存Contextのフィールドだけから構築される（捏造なし）", () => {
  ...
  const packet = buildExecutiveBriefingPacket({ context, previousQuarter: null, playerDraft: null, contracts: [] });
...
test("AMM-16: 同一入力からは常に同一Packetが得られる（決定論的）", () => {
  const context = minimalContext();
  const a = buildExecutiveBriefingPacket({ context, previousQuarter: null, playerDraft: null, contracts: [] });
  const b = buildExecutiveBriefingPacket({ context, previousQuarter: null, playerDraft: null, contracts: [] });
