...
// 【Phase 9 SAI-1で抽出】availableRawMaterialQuantity・pipelineRawMaterialQuantity・
// outstandingContractQuantity(ByProduct)・finishedGoodsByProduct・totalProductCapacity・
// totalCommonProcessingCapacity・ratioAdjustmentToUsd・referencePricesByMarketProduct・
// marketPremiumsFromPublicInfo は ./decisionHelpers.ts へ移動した（standardAi/ と共有するため。
// 挙動は一切変えていない、単純な切り出し）。
...
// 【Phase 9 SAI-1で抽出】buildCostExpectation は ./decisionHelpers.ts へ移動した
// （standardAi/ と共有するため。アーキタイプに依存しないロジックなので挙動は変えていない）。
