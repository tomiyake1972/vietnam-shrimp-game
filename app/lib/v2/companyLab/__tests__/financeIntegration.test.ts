...
test("受入確認FI-10: 現金不足が発生した場合、資金注入なしで負の現金とcashShortfallフラグが記録される（重要テスト20）", () => {
  ...
  // 【Phase 6C】Test16 の正式営業能力モデル切替（会社営業組織モデル v1）と
  // 営業能力SSoT統合により、この8ターンfixtureでは MASS の資金不足が発生しなくなった。
  // 過剰生産のための運転資金需要が消えたためであり、退行ではなく改善である。
  //
  // 「資金不足が発生したときに、勝手な資金注入で隠さず記録する」という重要テスト20の
  // 本体は、finance/__tests__/quarterClose.ts の受入確認Q-20（現金がマイナスになる
  // 入力を直接与える単体テスト）が担保している。ここでは実ランにおける不変条件
  // （フラグが立つときは必ず現金が負で、不足額が一致する）だけを検証する。
  assert.equal(
    shortfallSeen,
    false,
    "この fixture で資金不足が復活した場合は、営業・生産・資金の因果が変わったことを意味するため調査すること"
  );
});
