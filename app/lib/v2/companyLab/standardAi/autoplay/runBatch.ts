...
export interface AutoplayBatchConfig {
  ...
  /** 【SAI-5A追加】trueの場合のみ市場・商品志向プロファイル（orientationProfile.ts）を適用する。 */
  ...
  for (const seed of config.seeds) {
    try {
      const caseResult = runAutoplayCase({
        ...
        marketProductOrientationEnabled: config.marketProductOrientationEnabled,
        heterogeneousInitialConditionsEnabled: config.heterogeneousInitialConditionsEnabled,
        productLifecycleEnabled: config.productLifecycleEnabled,
        salesBaseAccumulationEnabled: config.salesBaseAccumulationEnabled,
        supplyPremiumFeedbackEnabled: config.supplyPremiumFeedbackEnabled,
        standardAiCapexEnabled: config.standardAiCapexEnabled,
      });
