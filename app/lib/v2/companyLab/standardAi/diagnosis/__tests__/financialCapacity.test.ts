...
test("financial capacity respects AR collection timing（帳簿上の売掛金全額を当期現金として扱わない）", () => {
  ...
  const result = buildStandardAiFinancialCapacity(observation2, pressures2, zeroFinancialCapacityPlan(), NEUTRAL_STANDARD_AI_COST_PROJECTION);
...
test("projected cash does not double-count receivables（当期新規の輸入発注はcash outに計上せず、決済期日到来分のみ計上する）", () => {
  ...
  const result = buildStandardAiFinancialCapacity(observation2, pressures2, plan, NEUTRAL_STANDARD_AI_COST_PROJECTION);
...
test("liquidity warningはprojectedCashAfterPlannedFinancingとtargetMinimumCashUsdの関係で決まる（3区分）", () => {
  const { observation2, pressures2 } = setupTurn2();
  const healthyResult = buildStandardAiFinancialCapacity(observation2, pressures2, zeroFinancialCapacityPlan(), NEUTRAL_STANDARD_AI_COST_PROJECTION);
  ...
  const negativeResult = buildStandardAiFinancialCapacity(observation2, pressures2, hugeCapexPlan, NEUTRAL_STANDARD_AI_COST_PROJECTION);
...
test("新規借入・任意期限前返済を計画に含めると、projectedCashAfterPlannedFinancingへ反映される", () => {
  const { observation2, pressures2 } = setupTurn2();
  const base = buildStandardAiFinancialCapacity(observation2, pressures2, zeroFinancialCapacityPlan(), NEUTRAL_STANDARD_AI_COST_PROJECTION);
...
test("スケールシナリオ分析: 10,000/15,000/17,000/20,000tでprojectedCashAfterPlannedFinancingが単調に変化する（規模が大きいほど現金消費が増える）", () => {
  ...
  const results = scales.map((scale) =>
    buildStandardAiFinancialCapacity(observation2, pressures2, scaledProductionPlan(baseMix, scale, { domesticRawPriceUsdPerKgOverride: 2.836 }), NEUTRAL_STANDARD_AI_COST_PROJECTION)
  );
