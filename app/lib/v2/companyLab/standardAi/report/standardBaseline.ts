...
function buildCandidate3Fixture(startPeriod: PeriodV2): CompanyFixture {
  return {
    ...
    // 【SAI-2最終化追加作業】感度分析（11点・12seed・全60社ケース完走）の結果、
    // 80人は経験的な最適点ではなく、90人のほうが8Qのmoderateさをほぼ維持しつつ
    // 累計収益性・32Qの深刻度でわずかに優れることを確認したため90人へ改めた
    // （上記コメント参照。80→90への変更は経験的な再評価であり、単なる据え置きではない）。
    salesForceHeadcountTotal: 90,
  };
}

const CANDIDATE_3: StandardBaselineCandidate = {
  id: "moderate-pressure",
  label: "候補3: moderate-pressure（候補2の操業条件＋財務を厳格化、営業工数ルール導入後に再校正・8Q基準のみ確定）",
  rationale:
    ...
    "16〜120人の感度分析（全60社ケース完走）の結果、80人は経験的な最適点ではなかった" +
    "ため90人へ改めた。ただし32Qのpaymentdefault率は90人でも63.3%と「57-100%は不適格」" +
    "という基準に抵触するため、32Qは未確定のまま。8Qのみを標準基準として確定する。",
