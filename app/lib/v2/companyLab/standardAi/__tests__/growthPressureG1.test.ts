...
import { assessGrowthPressure, GrowthPressureInput } from "../growth";
...
function input(overrides: Partial<GrowthPressureInput> = {}): GrowthPressureInput {
  return {
    companyId: "BAL",
    turn: 8,
    ...
    commercialAmbition: ambition(10000),
    commercialCommitment: commitment(10000),
    attainableProfitableTons: 40000,
    ...
    unservedOpportunity: unserved(),
    salesHiringZeroReason: null,
...
test("SAI-GROW-1: Vision gapがVision参照規模と現在規模から正しく計算される", () => {
  const a = assessGrowthPressure(input({ visionReferenceScaleTons: 20000, commercialAmbition: ambition(15000) }));
  ...
  const ahead = assessGrowthPressure(input({ visionReferenceScaleTons: 20000, commercialAmbition: ambition(26000) }));
  assert.equal(ahead.strategicScaleGapRatio, 0);
  // Visionが無い会社ではgap=0（架空のVisionを作らない）。
  const noVision = assessGrowthPressure(input({ visionReferenceScaleTons: null, growthAmbition: null }));
...
test("SAI-GROW-2: 市場機会が無ければpressureはLOWになり、制約はNO_MARKET_OPPORTUNITY", () => {
  const a = assessGrowthPressure(input({ attainableProfitableTons: 0 }));
  ...
  const equal = assessGrowthPressure(input({ attainableProfitableTons: 10000, commercialCommitment: commitment(10000) }));
  assert.equal(equal.opportunitySupport, 0);
  assert.equal(equal.level, "LOW");
});

test("SAI-GROW-3: 大きな市場機会＋Vision gapがあればpressureが高くなる", () => {
  const a = assessGrowthPressure(
    input({ visionReferenceScaleTons: 60000, commercialAmbition: ambition(10000), attainableProfitableTons: 60000 })
  );
...
test("SAI-GROW-4: margin brake — 期待貢献が最低水準以下ならpressureは0になる", () => {
  const base = assessGrowthPressure(input({ visionReferenceScaleTons: 60000, attainableProfitableTons: 60000 }));
  const weak = assessGrowthPressure(
    input({ visionReferenceScaleTons: 60000, attainableProfitableTons: 60000, weightedContributionUsdPerKg: 0.04 })
  );
  ...
  const unknownPrice = assessGrowthPressure(
    input({ visionReferenceScaleTons: 60000, attainableProfitableTons: 60000, weightedContributionUsdPerKg: 0, priceObservationMissing: true })
  );
  assert.equal(unknownPrice.marginSignal, 1);
});

test("SAI-GROW-5: inventory brake — 完成品在庫が過剰ならpressureが下がる", () => {
  const normal = assessGrowthPressure(input({ visionReferenceScaleTons: 60000, attainableProfitableTons: 60000 }));
  const full = assessGrowthPressure(
    input({
  ...
  const partial = assessGrowthPressure(
    input({
      visionReferenceScaleTons: 60000,
      attainableProfitableTons: 60000,
...
test("SAI-GROW-6: crisis brake — SEVERE_DISTRESSでpressureは0、LIQUIDITY_STRESSで減衰", () => {
  const normal = assessGrowthPressure(input({ visionReferenceScaleTons: 60000, attainableProfitableTons: 60000 }));
  const severe = assessGrowthPressure(
    input({ visionReferenceScaleTons: 60000, attainableProfitableTons: 60000, crisisState: "SEVERE_DISTRESS" })
  );
  const stress = assessGrowthPressure(
    input({ visionReferenceScaleTons: 60000, attainableProfitableTons: 60000, crisisState: "LIQUIDITY_STRESS" })
  );
...
test("SAI-GROW-7: healthy forward backlogは成長のpositive signalになる", () => {
  const none = assessGrowthPressure(input());
  const healthy = assessGrowthPressure(
    input({
...
test("SAI-GROW-8: overdueはpositive growth signalとして扱わない", () => {
  const overdue = assessGrowthPressure(
    input({
...
test("SAI-GROW-9: 営業能力不足はCOMMERCIAL → SALES_HIRINGへルーティングされる", () => {
  const a = assessGrowthPressure(
    input({ visionReferenceScaleTons: 60000, attainableProfitableTons: 60000, unservedOpportunity: unserved({ blockedBySalesCapacityTons: 2000 }) })
  );
...
test("SAI-GROW-11: 原料制約はRAW_MATERIAL → PROCUREMENT_EXPANSION", () => {
  const a = assessGrowthPressure(
    input({ visionReferenceScaleTons: 60000, attainableProfitableTons: 60000, unservedOpportunity: unserved({ blockedByRawMaterialTons: 1500 }) })
  );
...
test("SAI-GROW-12: 労働制約はLABOR → WORKFORCE_EXPANSION", () => {
  const a = assessGrowthPressure(
    input({ visionReferenceScaleTons: 60000, attainableProfitableTons: 60000, unservedOpportunity: unserved({ blockedByLaborTons: 900 }) })
  );
...
test("SAI-GROW-14: 同一状況でもgrowthAmbitionによりpressureの強さが変わる（会社差）", () => {
  ...
  const high = assessGrowthPressure(input({ ...shared, growthAmbition: "HIGH" }));
  const medium = assessGrowthPressure(input({ ...shared, growthAmbition: "MEDIUM" }));
  const low = assessGrowthPressure(input({ ...shared, growthAmbition: "LOW" }));
...
test("SAI-GROW-16: 既存の0.35 / 0.5 capは変更されていない", () => {
  ...
  // Shadowは0.35/0.5を機会評価に使わず、engineのsupplier shareまでを機会として数える。
  const a = assessGrowthPressure(input({ attainableProfitableTons: 60000, commercialCommitment: commitment(27704) }));
  assert.equal(a.observedOpportunityTons, 60000);
  assert.equal(a.currentSubmissionCeilingTons, 27704);
  assert.equal(a.ceilingSuppressedOpportunityTons, 60000 - 27704);
});
...
test("SAI-GROW-17: Sales Capacity Engineの式は変更されておらず、漸近上限は診断のみに使う", () => {
  ...
  const beyond = assessGrowthPressure(
    input({ visionReferenceScaleTons: 120000, observation: observation({ lastQuarterActualProductionByProduct: { hoso: 10000 } }) })
  );
  ...
  const vapHeavy = assessGrowthPressure(
    input({ visionReferenceScaleTons: 45000, observation: observation({ lastQuarterActualProductionByProduct: { vap: 10000 } }) })
  );
  assert.ok(vapHeavy.reasonCodes.includes("GROWTH_BLOCKED_BY_ENGINE_CAP"));
  const within = assessGrowthPressure(
    input({ visionReferenceScaleTons: 30000, observation: observation({ lastQuarterActualProductionByProduct: { hoso: 10000 } }) })
  );
...
test("SAI-GROW-20: Vision shadow sweep（候補Vision）はDecisionを一切変えない", () => {
  ...
  for (const r of results) {
    ...
    const candidate = assessGrowthPressure({
      ...
      commercialCommitment: r.diagnostics.commercialCommitment!,
