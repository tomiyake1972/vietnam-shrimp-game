...
test("CE1-4: 会社の商品志向（既存のproductOrientationMultipliers）がPD機械化への戦略適合をソフトに引き上げる（JPQ=japanQuality志向を例に、強制ルールではなく既存パラメータの自然な帰結として）", () => {
  ...
  const baselineResult = buildStandardAiCapexDecision(fixture, obsBorderline, pressures(), NO_SHORTFALL, 6000, STANDARD_AI_PARAMETERS_V1, undefined, NEUTRAL_STANDARD_AI_COST_PROJECTION);
  assert.ok(
  ...
  const biasedResult = buildStandardAiCapexDecision(fixture, obsBorderline, pressures(), NO_SHORTFALL, 6000, japanQualityLikeParams, undefined, NEUTRAL_STANDARD_AI_COST_PROJECTION);
...
test("CE1-10: PDライン増設（能力増強）とPD機械化（労務生産性）は同じ四半期に共存でき、二重計上・排他扱いにならない", () => {
  ...
  const result = buildStandardAiCapexDecision(fixture, obs, pressures({ equipmentUtilizationLastQuarter: 0.97, laborUtilizationLastQuarter: 0.97 }), HIGH_PD_SHORTFALL, 12000, STANDARD_AI_PARAMETERS_V1, undefined, NEUTRAL_STANDARD_AI_COST_PROJECTION);
...
test("CE1-11: PD機械化の想定経済効果（労務生産性の変化）は既存のeffectiveEfficiencyPerHeadTons由来であり、ゼロにならない", () => {
  ...
  const proposal = buildStandardAiCapexDecision(
    ...
    6000,
    STANDARD_AI_PARAMETERS_V1
  , undefined, NEUTRAL_STANDARD_AI_COST_PROJECTION).diagnostics.find((d) => d.code === "PD_MECH_PROPOSED");
...
test("CE1-13: 同一入力に対するPD機械化を含むCAPEX判断は決定論的である（再実行しても同じ結果）", () => {
  ...
  const run1 = buildStandardAiCapexDecision(fixture, obs, pressures(), NO_SHORTFALL, 6000, STANDARD_AI_PARAMETERS_V1, undefined, NEUTRAL_STANDARD_AI_COST_PROJECTION);
  const run2 = buildStandardAiCapexDecision(fixture, obs, pressures(), NO_SHORTFALL, 6000, STANDARD_AI_PARAMETERS_V1, undefined, NEUTRAL_STANDARD_AI_COST_PROJECTION);
