...
import { buildMechanizationLevelsByFactory } from "../pdMechanizationState";
...
function buildFactoryObservations(fixture: CompanyFixture, ownState: CompanyOwnState, period: PeriodV2): readonly FactoryObservation[] {
  ...
  // 【PD省人化投資の接続】工場ごとの機械化レベル。エンジン側
  // （runner.ts の mechanizationLevelByFactoryId）とまったく同じ関数を使い、
  // 判断側が独自の推定式を持たないようにする。
  const mechanizationLevelByFactory = buildMechanizationLevelsByFactory(
  ...
  return fixture.factories.map((f) => {
    ...
    return {
      ...
      ...(mechanizationLevelByFactory.has(f.factoryId) ? { mechanizationLevel: mechanizationLevelByFactory.get(f.factoryId) } : {}),
    };
  });
}
