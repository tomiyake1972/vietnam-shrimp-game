...
import {
  ALL_COMPANIES,
  CLI_DECISION_PROVIDER_KINDS,
  CLI_OUTPUT_FORMATS,
  CliArgumentError,
  CliDecisionProviderKind,
  CliOutputFormat,
  ParsedCompanyLabCliArgs,
} from "./types";

const KNOWN_FLAGS = new Set(["--scenario", "--mode", "--seed", "--turns", "--format", "--company", "--ai", "--help", "-h"]);
...
function tokenizeArgs(argv: readonly string[]): RawArgs {
  ...
  while (i < argv.length) {
    ...
    if (!KNOWN_FLAGS.has(flag)) {
      throw new CliArgumentError(
        `不明な引数です: "${argv[i]}"。使用可能な引数は --scenario, --mode, --seed, --turns, --format, --company, --ai, --help です。`
      );
    }
  ...
  return { help, scenario, mode, seed, turns, format, company, ai };
}
...
function validateAi(raw: string | undefined): CliDecisionProviderKind {
  const ai = raw ?? "auto-policy";
  if (!(CLI_DECISION_PROVIDER_KINDS as readonly string[]).includes(ai)) {
    throw new CliArgumentError(
      `--ai は ${CLI_DECISION_PROVIDER_KINDS.map((k) => `"${k}"`).join(" / ")} のいずれかである必要があります（受け取った値: "${ai}"）。`
    );
  }
  return ai as CliDecisionProviderKind;
}
...
export function parseCompanyLabCliArgs(argv: readonly string[]): ParsedCompanyLabCliArgs {
  ...
  const ai = validateAi(raw.ai);

  return { help: false, scenario, mode, seed: raw.seed.trim(), turns, format, company, ai };
}
...
使い方:
  npm run v2:company-simulate -- --scenario <id> --mode <canonical|variation> --seed <文字列> --turns <1以上の整数> --format <summary|json|csv|pricing-csv> [--company <all|会社ID>] [--ai <auto-policy|standard-ai>]
...
引数:
  ...
  --ai <種類>       【SAI-1】"auto-policy"（既定値。従来のアーキタイプ別暫定自動方針）または
                     "standard-ai"（会社IDによる分岐を一切行わない5社共通の標準AI。自動テストプレイ用）
...
例:
  ...
  npm run v2:company-simulate -- --scenario baseline --seed company-demo-001 --turns 8 --ai standard-ai --format summary
`;
}
