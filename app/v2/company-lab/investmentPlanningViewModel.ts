...
export const PROJECT_EFFECT_DISCLOSURES: Readonly<Record<CapitalProjectType, ProjectEffectDisclosure>> = {
  ...
  pdMechanization: {
    implementedEffects: [
      "稼働開始後、習熟度（ランプアップ）×当期のPD稼働率に応じたPDの実効労務負荷係数の低減（必要Worker人数の削減）",
      "PDの基準操業品質（歩留まり・規格外率・重大事故率）への小さな加点（上限付き、労務負荷係数の低減より小さい副次効果）",
      "取得原価の固定資産振替と、建物・機械コンポーネント別の減価償却",
      "稼働開始後の四半期固定保守費",
      "工場スペースの占有",
    ],
    notImplementedEffects: ["PD生産能力（トン/四半期）そのものの増加（これは「PD加工ライン増設」の効果です）", "工場別の機械化レベルの個別管理（現状は会社単位）"],
  },
};
