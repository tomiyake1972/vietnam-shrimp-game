...
> **【2026-07-29訂正】** 上記の「回復メカニズムが存在しません」という表現は技術的に不正確でした。`underwritingFrozen`は`financing/borrowingCapacity.ts`が`creditTier`・`severeArrears`・`insolvent`から**四半期ごとに再計算する値であり、永久フラグとして保持されているわけではありません**。延滞四半期数のカウンタ（`liquidityClose.ts`の`consecutiveArrearsQuarters`）も、延滞のない四半期が来れば0にリセットされる設計です。したがって、ゲームルール自体が「一度発生したら仕様上永久に解除しない」と規定しているのではありません。正確には、**信用悪化により融資余力がゼロとなり、その後の操業停止と固定費流出によって、自力では解除条件へ戻れない事実上の吸収状態**（融資余力ゼロ→原料調達不能→生産・売上ゼロ→延滞解消に必要な返済原資が生まれない→延滞カウンタがリセットされない、という自己強化ループの結果）です。この財務ルール自体は今回変更しておらず、32Q標準条件の設計課題として別途申し送ります（詳細は§11.10）。
...
> **【2026-07-29訂正】** 上記の「90〜120人（$22.8M〜$28.3M）に劣後」という要約は不正確でした。§11.3.1の表を個別に見ると、90人は**-$23,925,695**、93人は**-$5,658,139**、95人は**-$7,250,952**であり、いずれも80人の**+$10,006,064**より明確に**悪化**しています。80人に対して累計営業利益・32Q現金の両方で明確に優れるのは**100〜120人のみ**です（90・93・95人は80人より劣ります）。「90〜120人」をひとつの範囲として括ったことで、90人（実際には最悪水準）があたかも100人（最良水準）に近い成績であるかのような誤読を招きました。この誤りが§11.4の結論に波及しています。訂正後の判定は§11.10を参照してください。

### 11.4 営業人員80人の妥当性の再評価

> **【2026-07-29訂正】** 本節の結論（末尾）は誤りを含んでいたため、§11.10で訂正しています。項目1〜4の分析自体（能力式の妥当性、増分人件費と増分利益の関係等）は有効な分析として残していますが、末尾の「90人へ改めた」という決定と、その理由づけ（「累計収益性で明確に優れ」）は§11.3.1の実データと矛盾しており撤回します。
...
**結論（当初版・誤り）: ~~80人を「新ルールに必要な合理的規模」として据え置くことは適切ではないと判断し、感度分析の結果に基づいて90人へ改めました~~**（`standardBaseline.ts`の`buildCandidate3Fixture`を更新済み）。~~90人は80人とほぼ同水準の8Qのmoderateさ（40.0% vs 36.7%）を保ちながら、累計収益性で明確に優れ、32Qの深刻度もやや軽減されています（63.3% vs 81.7%）。~~

> **【2026-07-29訂正】** 「90人は累計収益性で明確に優れる」は誤りです。実際には90人の32Q累計営業利益（-$23.9M）・32Q現金平均（-$10.9M）は80人（+$10.0M・+$5.0M）より明確に**悪化**しています（32Q発生率が90人で低い〔63.3% vs 81.7%〕という一点のみを見て「優れる」と判断し、収益性・現金という他の指標を確認しないまま結論づけた、分析の統合段階での判断ミスです。§11.3.1の表自体・§11.1〜§11.3の集計方法そのものに誤りはありませんでした）。8Q基準を含めた再判定の結果、**営業人員は90人から80人へ差し戻します**。詳細な理由は§11.10を参照してください。
...
> **【2026-07-29訂正】** 本節の検証は、当時の（誤った）判断に基づき営業人員90人で実施したものです。営業人員を80人へ差し戻した後の§11.10でも、この節の結論（財務パラメータもまた閾値依存・非単調であること）自体は妥当であり、営業人員の値によって覆るものではないため、実測値はそのまま残します。
...
> **【2026-07-29訂正】** 本節は営業人員90人を前提とした当初の判定です。§11.4の誤り（90人が累計収益性で優れるとした点）を引き継いでおり、下記の「候補3（営業人員90人）を8Qシナリオの標準基準として確定」という結論は撤回します。訂正後の最終判定（営業人員80人への差し戻し、8Q・32Qの両基準に基づく理由）は§11.10を参照してください。「32Qは今回確定せず、別課題として切り出す」という判断自体は訂正後も変わらず有効です。

**判定（当初版）: 「2. 8Q基準としてのみ確定可能」とします。**
...
- 根本原因（§11.3.2・§11.5）は営業工数ルールそのものではなく、既存の`underwritingFrozen`吸収状態（信用悪化により融資余力がゼロとなり、その後の操業停止と固定費流出によって自力では解除条件へ戻れなくなる事実上の吸収状態、§11.3.2訂正参照）という、より広いゲームバランス上の設計にあり、今回のスコープ（営業工数のみ）で変更すべきではないと判断しました。
...
## 11.10 【SAI-2最終訂正】8Q標準営業人数の再判定とレポート整合

対象指示: 「SAI-2最終訂正：8Q標準営業人数の再判定とレポート整合」。三宅さんより、§11.4/§11.7で「90人は80人より累計収益性で明確に優れる」とした結論が、§11.3.1自身のデータ（90人: 32Q累計営業利益-$23.9M・32Q現金平均-$10.9M、80人: 同+$10.0M・+$5.0M）と矛盾しているとの指摘を受け、再検証・再判定を行いました。§10までの営業工数ルール実装・浮動小数点許容誤差修正・32Q全60ケース完走・13点感度分析・underwritingFrozen関連の長期悪化構造の特定・全1687テスト等の検証成功・「32Qは今回確定せず別課題とする」という判断は、いずれも指摘対象外として維持します。

### 11.10.1 矛盾の原因（再検証結果）

§11.3.1の表そのもの、および12seed×5社・全60ケースを対象とする集計方法（完走可否によるフィルタなし、default発生ケースも平均に含める）を`/tmp/sens_80.json`・`/tmp/sens_90.json`から再計算し、表の数値自体は正しいことを確認しました。**したがって、表の誤記（typo）ではありません。**

誤りの所在は、レポート執筆時の統合（narrative）段階にありました。90人は32Q paymentdefault**発生率**が80人より低い（63.3% vs 81.7%）という一点だけを根拠に「優れる」と判断し、同じ表に載っている累計営業利益・現金平均という、より直接的な収益性・財務健全性の指標を照合しないまま結論を書いてしまいました。§11.3節本文（訂正前）でも「累計営業利益（§11.3.1）で見ると、営業人員80人（$10.0M）は90〜120人（$22.8M〜$28.3M）に劣後」という記述で90〜120人をひとつの範囲にまとめていましたが、90・93・95人は実際には**負の値**（80人より悪化）であり、100〜120人のみが明確に優れています。この範囲のまとめ方が誤読を助長し、§11.4の結論に波及しました。

**結論: 表・集計方法は正しく、90人優位という判断は分析の統合（narrative）段階での誤りでした。** §11.3.1〜§11.3.2・§11.6の実測データ自体は訂正の必要がないため、そのまま維持しています（訂正箇所は本文中に注記のかたちで残しています）。

### 11.10.2 8Q基準での80・85・90人比較

directive項目2の指示（「8Q基準の選定には8Q時点のKPIを用いる」「必要であれば85人も追加」）に基づき、候補3の操業・財務条件を固定したまま営業人員のみ80/85/90人で振り、12seed×5社=60ケースを8ターン実行して比較しました（`scripts/_diag_sai2_8q_compare.ts`、実行後削除）。

| 営業人員 | 8Q paymentDefault発生率(n=60) | 8Q累計売上平均 | 8Q累計粗利平均 | 8Q累計営業利益平均 | 8Q営業人件費(会社当たり) | turn8現金平均 | turn8借入残高平均 | turn8追加融資余力平均 | turn8完成品在庫平均 | 効果制約scaleFactor実測範囲(該当件数) |
|---|---|---|---|---|---|---|---|---|---|---|
| 80 | **36.7%** | $367,919,964 | **$27,706,560** | **$8,162,114** | $5,120,000 | $11,080,596 | $34,566,643 | $3,440,679 | 896.8 | 0.999999（n=16） |
| 85 | **100.0%**（全60ケース・全5社で例外なく発生） | $369,924,304 | $26,724,928 | $6,806,328 | $5,440,000 | $3,701,532 | $31,049,762 | $1,360,577 | 1,791.2 | 0.999996〜0.999998（n=184） |
| 90 | **40.0%** | $369,926,757 | $25,643,207 | $5,392,610 | $5,760,000 | $11,400,376 | $32,523,824 | **$6,015,101** | 1,014.4 | 0.999992〜0.999998（n=256） |

会社別内訳（8Q発生率）:
- 80人: BAL 4/12・MASS 4/12・JPQ 4/12・VAP 6/12・CONSV 4/12（33〜50%、会社間で偏りは小さい）
- 85人: BAL 12/12・MASS 12/12・JPQ 12/12・VAP 12/12・CONSV 12/12（**全5社が例外なく100%**）
- 90人: BAL 5/12・MASS 5/12・JPQ 5/12・VAP 4/12・CONSV 5/12（33〜42%、会社間で偏りは小さい）

turn8時点の主要商品別販売数量（12seed×5社平均）は80人でhoso 4,518.6t・pd 2,823.2t・vap 2,177.9t、90人でhoso 4,628.3t・pd 3,072.2t・vap 2,257.8t で、いずれも数%程度の差に留まります。

**発見1: 営業人員85人は8Qで全60ケース・全5社が例外なく`paymentDefaultBy8Q`となる、非連続的な異常値です。** 80人（36.7%）・90人（40.0%）という前後の値と比較すると単調な傾向から著しく外れており、かつ特定の1社に偏るのではなく5社すべてで一様に100%となっている点で、単一会社のノイズではなくシステム全体の閾値通過（§11.3.2で特定した`underwritingFrozen`吸収状態）に起因すると判断しました。85人は個別トレースで見ると健全な財務指標を示すケースも含まれており（`paymentDefaultBy8Q`は「8Q中に一度でも延滞ストリークが発生したか」を意味する経過フラグであり、turn8時点の健全性そのものを意味しません）、この統計だけで85人を「危険」と断定するのは早計ですが、少なくとも85人を「80と90の中間の穏当な選択肢」として採用する根拠にはなりません。**85人は本比較から除外します。**

**発見2: 営業工数制約（`SALES_PLAN_REDUCED_FOR_EFFORT_CAPACITY`）の理由コードは80〜90人のいずれでも発火するものの、実際の数量的な効果（scaleFactor）は0.999992〜0.999999の範囲、すなわち縮小率0.01%未満に留まります。** `applyMarketSalesEffortCapacity`（`sales/marketEffort.ts` L152-190）は真に制約が拘束した市場のみをadjustments配列へ記録するため、制約発火件数（80人=16件・85人=184件・90人=256件）は headcount に応じて大きく変動しますが、これは標準AI自身が能力式`C(h)`に合わせて事前に販売計画を自己調整しているためで、80〜90人のいずれの水準でも「営業工数制約が経営判断に量的に大きな影響を与えている」とまでは言えません。この点は directive項目3の基準(b)（「営業工数制約が実際に経営判断へ意味のある影響を与える」）を厳密には強く満たさない、正直な限界として申し送ります。

### 11.10.3 headcount再判定（directive項目3の基準に照らして）

- **(a) 8Qで穏当な圧力が発生するか**: 80人（36.7%）・90人（40.0%）はいずれも「全滅でも安全すぎでもない」水準を満たします。85人は(c)に抵触するため除外します。
- **(b) 営業工数制約が経営判断に意味のある影響を与えるか**: §11.10.2のとおり、80〜90人のいずれでも量的な影響（scaleFactor）は僅少（<0.01%）です。この点は80/90いずれを選んでも同程度であり、決定的な弁別要因にはなりません（正直な限界として記録）。
- **(c) 全社・全seedが即座に破綻するほど過酷ではないか**: 85人は全60ケース・全5社が8Qで例外なくdefaultとなるため抵触し、除外します。80人・90人はいずれも一部のケースに留まり抵触しません。
- **(d) 営業人件費を含め標準的な会社として説明可能か**: 80人=$5,120,000/8Q（会社当たり）、90人=$5,760,000/8Q。いずれも`salesForceCost = headcount × $8,000/人/四半期`の理論値と厳密に一致しており、双方とも説明可能です。
- **(e) default率を目標値へ合わせるためだけに選ばないこと**: 80人と90人の8Q発生率（36.7% vs 40.0%）はほぼ同水準であり、この基準だけでは弁別できません。したがって、他の8Q KPI（累計粗利・累計営業利益）と、既に確定している32Q累計収益性・現金の両方を判断材料とします。

8Qの範囲では、80人は90人に対して累計粗利（+$27.7M vs $25.6M）・累計営業利益（+$8.2M vs $5.4M）でわずかに優れ、発生率も同程度に低く（36.7% vs 40.0%）、90人が優位なのはturn8時点の追加融資余力（$6.0M vs $3.4M）のみです。32Qでは§11.3.1のとおり80人が累計営業利益（+$10.0M vs -$23.9M）・現金平均（+$5.0M vs -$10.9M）のいずれでも明確に優れています。90人を選ぶ根拠として8Q・32Qのどちらの基準を見ても「収益性で明確に優れる」という主張は成立せず、90人固有の優位性は追加融資余力という限定的な指標に留まります。

**最終判定: 候補3の営業人員を90人から80人へ差し戻します。** 85人は(c)の基準（8Qで全滅同然）により明確に不適格として除外し、80人と90人は8Q・32Qいずれの基準でもほぼ同等かわずかに80人が優れるため、より単純で§9〜§10の既存分析（80人時点のトレース・既存5社との比較）との整合性も高い80人を採用しました。**単にdefault率を目標値に合わせるための選択ではなく**、8Q・32Q双方の収益性・現金指標を総合した結果です。90人を維持しない理由は「収益性が劣るため」であり、以前の「累計収益性で明確に優れる」という記述の裏返しではなく、正しい根拠に基づく独立した再判定です。

### 11.10.4 `underwritingFrozen`の記述訂正（財務ルール自体は変更せず）

§11.3.2・§11.5・§11.7の該当箇所に本文中で訂正注記を追加済みです。要点:

- `financing/borrowingCapacity.ts` L58: `underwritingFrozen = creditTier === "E" || severeArrears || insolvent` — **四半期ごとに現在の信用条件から再計算される値**であり、永久フラグとして保持されているわけではありません。
- `financing/liquidityClose.ts` L525: `consecutiveArrearsQuarters`は延滞のない四半期が来れば0にリセットされる設計です。
- したがって、ゲームルールが「一度発生したら仕様上永久に解除しない」と規定しているのではなく、正確には**「信用悪化により融資余力がゼロとなり、その後の操業停止と固定費流出によって、自力では解除条件へ戻れない事実上の吸収状態」**です（融資余力ゼロ→原料調達不能→生産・売上ゼロ→延滞解消に必要な返済原資が生まれない→延滞カウンタがリセットされない、という自己強化ループ）。
- 指示のとおり、**この財務ルール自体は今回変更していません**。32Q標準条件の設計課題（§11.5の代替案A/B等）として引き続き申し送ります。

### 11.10.5 レポート・コードの整合

- `standardBaseline.ts`の`buildCandidate3Fixture`（`salesForceHeadcountTotal`）を90→80へ差し戻し、`CANDIDATE_3`の`label`・`description`・`rationale`を本節の判定に合わせて更新しました（§11.11参照）。
- 本レポートの§11.3.2・§11.4・§11.5・§11.6・§11.7には、旧記述を削除せず、誤りの箇所に「【2026-07-29訂正】」形式の注記を追加し、本節（§11.10）を参照するかたちで整合させました。
- `docs/development_diary_2026-07-29.md`に本訂正の経緯を追記します（§11.12参照）。

### 11.10.6 32Qの取り扱い

directive・§11.7（当初版）と同様、**32Qの標準条件としての最終確定は今回も見送ります**。90→80への差し戻し後も、80人の32Q paymentdefault発生率は81.7%であり「57〜100%は不適格」という基準に抵触するため、32Qは引き続き未確定のまま、`underwritingFrozen`まわりの設計変更（§11.5の代替案A/B等）の要否についての別途のご判断を仰ぐ課題として切り出します。**8Qのみを標準基準として確定します。**

## 11.11 変更ファイル（本訂正分）

| ファイル | 変更内容 |
|---|---|
| `companyLab/standardAi/report/standardBaseline.ts` | 候補3の`salesForceHeadcountTotal`を90→80へ差し戻し。`label`・`description`・`rationale`・関連コメントを§11.10の判定に合わせて更新 |
| `docs/v2/reports/sai2_standard_baseline_report.md` | §11.3.2・§11.4・§11.5・§11.6・§11.7に訂正注記を追加（旧記述は削除せず維持）。§11.10〜§11.12を追加 |
| `docs/development_diary_2026-07-29.md` | 本訂正の経緯を追記（§8） |
| `scripts/_diag_sai2_8q_compare.ts` | 8Q比較用の一時調査スクリプト（検証後に削除、コミット対象外） |

## 11.12 検証・commit・push（本訂正分）

- SAI-2標準基準テスト（`standardBaseline.test.ts`7件）: 全件成功（営業人員80人への差し戻し後も影響なし）。
- 営業工数関連テスト（`marketEffort.test.ts`・`salesEffort.test.ts`・`salesEffortRoundtrip.test.ts`合計85件）: 全件成功。
- 丸め誤差関連テスト（`headcountBudgetTolerance`系4件を含む`quarterClose.test.ts`）: 全件成功（上記85件に含む）。
- 全テストスイート（`npm test`）: **1687件**全件成功（件数はc39db0e時点から変わらず）。
- `npx tsc --noEmit`: エラーなし。
- `npm run lint`: エラーなし（既存の無関係な警告2件のみ、c39db0e時点と同一）。
- `npx next build`: 成功。
- スクラッチ調査スクリプト（`scripts/_diag_sai2_8q_compare.ts`）は検証後に削除済み（コミット対象外）。
- `feature/v2-sai2-standard-baseline`ブランチへcommit・push済み（コミットIDは本レポートを含むコミット自体。`git log`参照）。**`develop/v2`へはまだマージしていません**。
