...
|---|---|---:|---:|---:|---:|:-:|
| raw_target_demand（旧） | pd | 0.268 | 0.171 | 0.393 | 1.361 | × |
| raw_target_demand（旧） | vap | 0.492 | 0.299 | 1.661 | 1.251 | × |
| addressable_demand 候補(i) | pd | 0.299 | 0.191 | 0.436 | 1.345 | × |
| addressable_demand 候補(i) | vap | 0.545 | 0.333 | 1.845 | 1.223 | × |
| neutral_baseline 候補(ii) | pd | 0.830 | 0.710 | 1.000 | 1.078 | ○ |
| neutral_baseline 候補(ii) | vap | 0.658 | 0.490 | 1.000 | 1.153 | × |
| **completed_supply 候補(iii)** | **pd** | **1.026** | 1.000 | 1.048 | 0.991 | **○** |
| **completed_supply 候補(iii)** | **vap** | **1.102** | 1.060 | 1.796 | 0.948 | **○** |

（分子は営業工数制約 `applyMarketSalesEffortCapacity` 適用**後**の提示量。縮小前の
希望量で数えると、市場へ出していない量まで「売れ残り」に計上され、成約側の母集団と
ずれるため。同じ理由で営業基盤の成約充足率の分母も適用後にそろえている。）
...
#### 次回の校正フェーズへの申し送り（Fable再監査の指摘）

採用した定義は「中心を1.0にそろえる」条件は満たすが、**振れ幅は商品・時間で
そろわない**。分母が全ベトナム対象需要のため、

- PD は 1.000〜1.048、VAP は 1.060〜1.796 と偏差スケールが商品で大きく異なり、
  共有しきい値（1.14 / 1.20）が事実上VAP専用になる
- ベトナム総需要が32Qで約2倍になるため、同じ売れ残りトン数が後期ほど小さい
  圧力として読まれる（VAPのEWMAが 1.32→1.08 へ低下した一因）

再監査は**分母を「5社の提示量」に取る**案（＝ `1 + 未成約量/提示量` ＝ 売れ残り率）を
提示している。この形なら商品間・時間方向ともスケール不変になり、今回の採用条件
（当てはめ定数なし・1.0中心・方向性・二重計上なし）もすべて満たす。**次回の校正
フェーズの第一候補**として記録する（今回は構造的因果が確認できているため変更しない）。

同様に、割安シグナル（§2.4）は稼働率倍率を意図的に残しているが、実測では
その項が支配的で（T1 VAP の cheapness +0.46 は100%が世界VAP加工能力の余剰に由来）、
「5社の供給行動→普及速度」という設計上の因果が外生のシナリオ能力トレンドに
覆われている。参照側にも同じ稼働率倍率を掛けて「5社起因の割安分だけ」を抽出する
案を、あわせて校正フェーズの検討事項として記録する。
