...
//
// 既存ManagementProfile（SAI-4）・CompanyOrientationProfile（SAI-5A）を
// Management Console本番経路（simulation/engine.ts の standardAiProfileMode）へ
// 接続した上で、PROFILE OFF vs PROFILE ON を同一scenario/seedで比較する。
// Profile値そのものは一切変更しない（SP-A1監査で発見済みの既存値をそのまま使う）。
//
// 出力:
//   docs/standard_ai/benchmarks/strategy_profile_spq1_summary.md
//   docs/standard_ai/benchmarks/strategy_profile_spq1_company.csv
//   docs/standard_ai/benchmarks/strategy_profile_spq1_investments.csv
//   docs/standard_ai/benchmarks/strategy_profile_spq1_product_mix.csv
//
// 使い方: npx tsx scripts/strategyProfileQuantificationSPQ1.ts
...
function runOne(scenario: string, seed: string, mode: "OFF" | "ON"): Map<string, CompanyRunAggregate> {
  let session = createSimulationSession({
    simulationRunId: `spq1-${scenario}-${seed}-${mode}`,
    scenarioId: scenario,
    seed,
...
async function main() {
  ...
  writeCsv(path.join(outDir, "strategy_profile_spq1_company.csv"), companyHeaders, companyRows);

  // investments.csv
  const investmentHeaders = ["scenario", "seed", "mode", "companyId", "projectType", "proposedCount"];
  const investmentRows: (string | number)[][] = [];
  for (const a of allAggregates) {
    for (const t of PROJECT_TYPES) {
      investmentRows.push([a.scenario, a.seed, a.mode, a.companyId, t, a.proposedByType[t] ?? 0]);
    }
  }
  writeCsv(path.join(outDir, "strategy_profile_spq1_investments.csv"), investmentHeaders, investmentRows);
  ...
  writeCsv(path.join(outDir, "strategy_profile_spq1_product_mix.csv"), mixHeaders, mixRows);

  // summary.md — OFF vs ON comparison per company, averaged across scenario x seed
  const lines: string[] = [];
  lines.push("# Strategy Profile Quantification SP-Q1 — OFF vs ON Summary (auto-generated)");
  lines.push("");
  ...
  fs.writeFileSync(path.join(outDir, "strategy_profile_spq1_summary.md"), lines.join("\n") + "\n", "utf8");
