...
export interface CaseRunOptions {
  ...
  readonly diagnosticCashGrantUsd: number;
  /**
   * 【加工品市場進化 §f】市場進化・営業能力再設計を有効にするか。
   * false = Phase6当時とまったく同じ条件（比較の基準）。
   * true  = config.sai5.productLifecycle + config.marketEvolution 一式をON。
   */
  readonly marketEvolutionEnabled: boolean;
  /**
   * 【追加検証】BALの提示価格をこの比率だけ引き下げる（0.03なら3%割引）。
   * 「新工場の能力増が契約へ通らないのは、営業能力ではなく**成約競争力**が
   * 律速だから」という仮説を検証するための操作。
   */
  readonly priceDiscountRatio?: number;
  /**
   * 【追加検証】BALが毎四半期この人数だけ営業人員を追加採用する
   * （companyLab/salesForceHiring.ts）。「新工場の能力増が契約へ通らないのは
   * 営業人員が律速だから」という仮説の検証用。
   */
  readonly salesForceHirePerQuarter?: number;
}
...
export function runCase(options: CaseRunOptions): readonly QuarterMetricsRow[] {
  const config = {
    ...
    seed: options.seed,
    turns: options.horizonQuarters,
    ...(options.marketEvolutionEnabled
      ? {
  ...
  for (let turn = 0; turn < options.horizonQuarters; turn++) {
    ...
    for (const f of fixtures) {
      ...
      if (f.companyId === FOCUS_COMPANY_ID) {
        ...
        if (options.salesForceHirePerQuarter && options.salesForceHirePerQuarter > 0) {
          decision = {
            ...decision,
            salesForceHireCount: options.salesForceHirePerQuarter,
          };
        }
        if (options.priceDiscountRatio && options.priceDiscountRatio > 0) {
          // 基準価格は市場側で決まるため、ここでは提示価格調整幅を負方向へ動かす。
          // 概算として、直近のHOSO基準価格 5.0 USD/kg 相当の割引額を用いる。
          const discountUsdPerKg = -5.0 * options.priceDiscountRatio;
          decision = {
            ...decision,
            salesPlans: decision.salesPlans.map((p) => ({
              ...p,
              priceAdjustmentUsdPerHosoEqKg: p.priceAdjustmentUsdPerHosoEqKg + discountUsdPerKg,
            })),
          };
        }
    ...
    rows.push({
      ...
      marketEvolutionEnabled: options.marketEvolutionEnabled,
...
function determineVariantForEnvironment(env: EnvironmentDefinition): "A" | "B" {
  const probeRows = runCase({ environment: env, caseType: "on-unstaffed", seed: SEED, horizonQuarters: HORIZON_QUARTERS, diagnosticCashGrantUsd: 0, marketEvolutionEnabled: false });
  const completed = probeRows.some((r) => r.newFactoryProjectStatus === "completed" || r.newFactoryOperational);
  return completed ? "A" : "B";
}

/**
 * 【加工品市場進化 §f】Phase6の「契約数量が全ケース・全環境で完全に同一（60,575t）」
 * という所見が、市場進化・営業能力再設計を有効にしたときに動くかを測る比較。
 *
 * 比較軸: 環境(3) × ケース(off/on-unstaffed/on-staffed) × 市場進化(OFF/ON)。
 * 市場進化OFFの側はPhase6当時とまったく同じ条件であり、再現の確認も兼ねる。
 */
export interface ComparisonRow {
  ...
  readonly totalSalesDesiredTons: number;
  readonly totalProducedTons: number;
  readonly cumulativeNetIncomeUsd: number;
  readonly finalCashUsd: number;
  readonly newFactoryOperationalQuarters: number;
}

const CASES: readonly ConstructionCase[] = ["off", "on-unstaffed", "on-staffed"];

export function runComparison(): readonly ComparisonRow[] {
  const rows: ComparisonRow[] = [];
  for (const marketEvolutionEnabled of [false, true]) {
    for (const env of ENVIRONMENTS) {
      for (const caseType of CASES) {
        const quarters = runCase({
          environment: env,
          caseType,
          seed: SEED,
        ...
        rows.push({
          ...
          caseType,
          marketEvolutionEnabled,
          totalContractedTons: quarters.reduce((s, r) => s + r.newContractedQuantityTons, 0),
          totalSalesDesiredTons: quarters.reduce((s, r) => s + r.salesDesiredQuantityTons, 0),
          totalProducedTons: quarters.reduce((s, r) => s + r.hosoProducedTons + r.pdProducedTons + r.vapProducedTons, 0),
          cumulativeNetIncomeUsd: quarters.reduce((s, r) => s + r.netIncomeUsd, 0),
          finalCashUsd: quarters.length > 0 ? quarters[quarters.length - 1].cashUsd : 0,
          newFactoryOperationalQuarters: quarters.filter((r) => r.newFactoryOperational).length,
        });
      }
    }
  }
...
/**
 * 【追加検証】需要が実際に伸びる長期horizon（32四半期＝ライフサイクル普及と
 * 産地別加工能力の弧が一巡する期間）で、新工場の能力増が契約へ通るかを見る。
 *
 * 16四半期の比較では、環境倍率がBAL自身の意思決定だけを拡大しており
 * **市場側の対象需要は3環境で同一**だったため、「需要が伸びたときに新工場が
 * 効くか」という本来の問いを測れていなかった。ここでは市場側の需要成長
 * （ライフサイクル普及＋シナリオの需要トレンド）に任せる。
 */
export function runLongHorizonComparison(quarters: number): readonly ComparisonRow[] {
  const rows: ComparisonRow[] = [];
  const env = ENVIRONMENTS.find((e) => e.id === "env3-constrained")!;
  for (const caseType of ["off", "on-staffed"] as const) {
    const quarterRows = runCase({
      environment: env,
      caseType,
      seed: SEED,
    ...
    rows.push({
      ...
      caseType,
      marketEvolutionEnabled: true,
      totalContractedTons: quarterRows.reduce((s, r) => s + r.newContractedQuantityTons, 0),
      totalSalesDesiredTons: quarterRows.reduce((s, r) => s + r.salesDesiredQuantityTons, 0),
      totalProducedTons: quarterRows.reduce((s, r) => s + r.hosoProducedTons + r.pdProducedTons + r.vapProducedTons, 0),
      cumulativeNetIncomeUsd: quarterRows.reduce((s, r) => s + r.netIncomeUsd, 0),
      finalCashUsd: quarterRows.length > 0 ? quarterRows[quarterRows.length - 1].cashUsd : 0,
      newFactoryOperationalQuarters: quarterRows.filter((r) => r.newFactoryOperational).length,
    });
  }
  return rows;
}

if (process.argv[1] && process.argv[1].endsWith("newFactoryConversionComparison.ts")) {
  const rows = runComparison();
  console.log("環境      | ケース        | 市場進化 | 契約数量(t) | 販売希望(t) | 生産(t)   | 累積純利益(USD)  | 期末現金(USD)");
  console.log("----------|---------------|----------|-------------|-------------|-----------|------------------|----------------");
  for (const r of rows) {
    console.log(
      `${r.environment.padEnd(9)} | ${r.caseType.padEnd(13)} | ${(r.marketEvolutionEnabled ? "ON" : "OFF").padEnd(8)} | ` +
        `${fmt(r.totalContractedTons).padStart(11)} | ${fmt(r.totalSalesDesiredTons).padStart(11)} | ${fmt(r.totalProducedTons).padStart(9)} | ` +
        `${fmt(r.cumulativeNetIncomeUsd).padStart(16)} | ${fmt(r.finalCashUsd).padStart(14)}`
    );
  }
  console.log("\n--- 契約数量がケース間で動いたか（同一環境・同一市場進化設定の中で比較）---");
  for (const marketEvolutionEnabled of [false, true]) {
    for (const env of ENVIRONMENTS) {
      const subset = rows.filter((r) => r.environment === env.id && r.marketEvolutionEnabled === marketEvolutionEnabled);
      const values = subset.map((r) => r.totalContractedTons);
      const spread = Math.max(...values) - Math.min(...values);
      console.log(
        `市場進化=${marketEvolutionEnabled ? "ON " : "OFF"} ${env.id.padEnd(16)}: ` +
          `off=${fmt(values[0])} on-unstaffed=${fmt(values[1])} on-staffed=${fmt(values[2])} → ケース間の差=${fmt(spread)}t`
      );
    }
  }
}

if (process.argv[1] && process.argv[1].endsWith("newFactoryConversionComparison.ts") && process.argv.includes("--long")) {
  console.log("\n=== 長期horizon（32四半期・env3-constrained・市場進化ON）===");
  const rows = runLongHorizonComparison(32);
  for (const r of rows) {
    console.log(
      `${r.caseType.padEnd(12)} 契約=${fmt(r.totalContractedTons).padStart(10)}t 生産=${fmt(r.totalProducedTons).padStart(10)}t ` +
        `累積純利益=${fmt(r.cumulativeNetIncomeUsd).padStart(15)} 期末現金=${fmt(r.finalCashUsd).padStart(14)} 稼働Q=${r.newFactoryOperationalQuarters}`
    );
  }
  const off = rows.find((r) => r.caseType === "off")!;
  const on = rows.find((r) => r.caseType === "on-staffed")!;
  console.log(
    `差分: 契約 ${fmt(on.totalContractedTons - off.totalContractedTons)}t (${(((on.totalContractedTons / off.totalContractedTons) - 1) * 100).toFixed(1)}%), ` +
      `純利益 ${fmt(on.cumulativeNetIncomeUsd - off.cumulativeNetIncomeUsd)}`
  );
}

if (process.argv[1] && process.argv[1].endsWith("newFactoryConversionComparison.ts") && process.argv.includes("--binding")) {
  console.log("\n=== 何が律速か（32四半期・env3-constrained・市場進化ON）===");
  const env = ENVIRONMENTS.find((e) => e.id === "env3-constrained")!;
  const variants: { label: string; caseType: ConstructionCase; discount: number }[] = [
    { label: "工場なし・割引なし", caseType: "off", discount: 0 },
    { label: "工場あり・割引なし", caseType: "on-staffed", discount: 0 },
    { label: "工場なし・3%割引", caseType: "off", discount: 0.03 },
    { label: "工場あり・3%割引", caseType: "on-staffed", discount: 0.03 },
  ];
  for (const v of variants) {
    const rows = runCase({
      ...
      diagnosticCashGrantUsd: DIAGNOSTIC_CASH_GRANT_USD,
      marketEvolutionEnabled: true,
      priceDiscountRatio: v.discount,
    });
    const contracted = rows.reduce((s2, r) => s2 + r.newContractedQuantityTons, 0);
    const produced = rows.reduce((s2, r) => s2 + r.hosoProducedTons + r.pdProducedTons + r.vapProducedTons, 0);
    const ni = rows.reduce((s2, r) => s2 + r.netIncomeUsd, 0);
    console.log(`${v.label.padEnd(22)} 契約=${fmt(contracted).padStart(10)}t 生産=${fmt(produced).padStart(10)}t 累積純利益=${fmt(ni).padStart(15)}`);
  }
}

if (process.argv[1] && process.argv[1].endsWith("newFactoryConversionComparison.ts") && process.argv.includes("--hiring")) {
  console.log("\n=== 営業人員が律速かの検証（32四半期・env3-constrained・市場進化ON）===");
  const env = ENVIRONMENTS.find((e) => e.id === "env3-constrained")!;
  const variants: { label: string; caseType: ConstructionCase; hire: number }[] = [
    { label: "工場なし・採用なし", caseType: "off", hire: 0 },
    { label: "工場あり・採用なし", caseType: "on-staffed", hire: 0 },
    { label: "工場なし・毎期+3人", caseType: "off", hire: 3 },
    { label: "工場あり・毎期+3人", caseType: "on-staffed", hire: 3 },
  ];
  for (const v of variants) {
    const rows = runCase({
      environment: env,
      ...
      diagnosticCashGrantUsd: DIAGNOSTIC_CASH_GRANT_USD,
      marketEvolutionEnabled: true,
      salesForceHirePerQuarter: v.hire,
    });
    const contracted = rows.reduce((s2, r) => s2 + r.newContractedQuantityTons, 0);
    const produced = rows.reduce((s2, r) => s2 + r.hosoProducedTons + r.pdProducedTons + r.vapProducedTons, 0);
    const ni = rows.reduce((s2, r) => s2 + r.netIncomeUsd, 0);
